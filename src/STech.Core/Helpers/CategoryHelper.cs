...
public class CategoryHelper
{
    public static List<CategoryWithChildren> MakeCategoriesHierarchy(List<Category> allCategories)
    {
        if (allCategories != null && allCategories.Count > 0)
        {
            ...
            {
                Category = item,
                Children = allCategories.Where(child => child.ParentCategoryID == item.ID).OrderBy(x => x.Name).ToList()
            }).ToList();
    ...
    {
        return parentCategoryID.HasValue ? allCategories.First(x => x.ID == parentCategoryID) : null;
    }
